...
MYSQL *db_connect_to_main(char *dbname)
{
	...
	if (!mysql_real_connect(db, "172.16.9.212", "root", "qwepoi123", dbname, 0, NULL, 0))
	{
...
// @return MYSQL *
//------------------------------------------------------------------------------
MYSQL *db_connect_to_backup(char *dbname)
{
	MYSQL *db = mysql_init(NULL);
	if (!db)
	{
...
// @return MYSQL *
//------------------------------------------------------------------------------
MYSQL *db_connect_to_local(char *dbname)
{
...
// @return MYSQL *
//------------------------------------------------------------------------------
MYSQL *db_connect_controaddb(char *dbname)
{
	//printf("db_connect: start! \n");
...
// @return MYSQL *
//------------------------------------------------------------------------------
MYSQL *db_connect_opdb(char *dbname)
{
	//printf("db_connect: start! \n");
	...
	if (!mysql_real_connect(db, "172.16.9.212", "root", "dlwjarhd1029", dbname, 0, NULL, 0))
	{
		printf("db_connect: mysql_real_connect failed(%s)\n", mysql_error(db));
		return NULL;
	}
