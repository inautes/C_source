...
int daemstat_process();
int daemstat_process_db();
int daemstat_process_init(int argc, char **argv);
int daemstat_process_term();
void daemstat_signal(int nSignal);
...
int daemstat_process()
{
	...
	if (strcmp(gst_date, "00000000") == 0)
	{
		...
		if (mysql_query(con, szQuery)){
		    ZzLOG(ERROR, "[daemstat]sysdate: mysql_query error...\n");
		...
		{
		    ZzLOG(ERROR, "[daemstat]sysdate: mysql_store_result error...\n");
			ZzLOG(ERROR, "[%d](%s)",mysql_errno(con), mysql_error(con));
			return -1;
		}
	 	if (mysql_num_rows(res)==0)
	 	{
		    ZzLOG(ERROR, "[daemstat]sysdate: mysql_num_rows error...\n");
			ZzLOG(ERROR, "[%d](%s)",mysql_errno(con), mysql_error(con));
			return -1;
		}
...
int daemstat_process_db()
{
    ...
    ZzLOG(ALWAY, "---------------------------------------------------------\n");
    ZzLOG(ALWAY, "[daemstat]gproc_date : [%s]\n", gst_date);  
    ZzLOG(ALWAY, "---------------------------------------------------------\n");
	...
	while(1) 
	{
		...
		if (mysql_query(con, szQuery))
		{
		    ZzLOG(ERROR, "daemstat_process: mysql_query error...\n");
			ZzLOG(ERROR, "[%d](%s)\n",mysql_errno(con), mysql_error(con));
			return -1;
		}

		if (!(res = mysql_store_result(con))) {
		    ZzLOG(ERROR, "daemstat_process: mysql_store_result error...\n");
			ZzLOG(ERROR, "[%d](%s)\n",mysql_errno(con), mysql_error(con));
			return -1;
		}
	 	if (mysql_num_rows(res)==0)	{
			mysql_free_result(res);
		    ZzLOG(ALWAY, "daemstat_process: 처리할 자료가 없습니다.\n");
			break;
		}
	...
	ZzLOG(ALWAY, "daemstat_process: 처리된 정보의 갯수는 %lu 입니다.\n",ulCount);
	return 0;

}

//******************************************************************************
//* daemstat_process_userdel
//*
//* 특정 사용자에 한하여 모든 데이타 삭제 처리 한다.
//******************************************************************************
int daemstat_process_userdel()
{
	char szQuery[1600];		// query string
	...
	if (daemstat_process_DBDelete(szQuery) != 0) return -1;
...
//* daemstat db 처리로직
//******************************************************************************
int daemstat_process_DBDelete(char* in_szQuery)
{
...
int daemstat_process_userchk()
{
	...
	select_cnt = daemstat_process_DBQuery(szQuery);
...
//* daemstat db 처리로직
//******************************************************************************
int daemstat_process_DBQuery(char* in_szQuery)
{
MYSQL_RES *loc_res;
MYSQL_ROW  loc_row;
int select_cnt;
...
int daemstat_process_init(int argc, char **argv)
{
    ...
    */
	...
	{
		ZzLOG(ERROR, "[daemstat]DB에 접속하지 못 하였습니다...\n");
		db_disconnect(con);
	   	return(-1); 
	}
...
{                
	...
	** SIGNAL 정의
	*/       
	signal(SIGTERM, daemstat_signal);
	signal(SIGINT,  daemstat_signal);
	signal(SIGQUIT, daemstat_signal);
	signal(SIGKILL, daemstat_signal);
	...
	if ( daemstat_process_init(argc, argv) == 0 ) {
		/* 프로그램 메인루틴 */
		rc = daemstat_process();
		/* 프로그램 종료루틴 */                    
		daemstat_process_term();
