...
//		ZzLOG(ERROR, "%s\n\n",szQuery);
		...
		if (!(res = mysql_store_result(con))) {
	 	...
	 	if (mysql_num_rows(res)==0)	{
			mysql_free_result(res);
		    ZzLOG(ALWAY, "[daem5502]daem5502_process: 처리할 자료가 없습니다.\n");
			break;
		}
...
//	    ZzLOG(ALWAY, "daem5502_process: select_cnt(%d)\n", mysql_num_rows(res));
		...
		while((row = mysql_fetch_row(res))) 
		{
			...
			if (tran_begin(con)!=0)	{
			    ZzLOG(ERROR, "[daem5502]tran_begin1: 테이베이스 오류입니다.\n");  
				return -1;
			}
							
			if (tran_begin(con_backup)!=0)	{
			    ZzLOG(ERROR, "[daem5502]tran_begin2: 테이베이스 오류입니다.\n");  
				return -1;
			}
			...
			if (tran_end(con_backup)!=0)	{
			    ZzLOG(ERROR, "[daem5502]process_db2: tran_end 테이베이스 오류입니다.\n");  
			    goto daem5502_process_db_err;
			}
			...
			//--------------------------------------------------------------------------
			if (tran_end(con)!=0)	{
			    ZzLOG(ERROR, "[daem5502]process_db1: tran_end 테이베이스 오류입니다.\n");  
			    goto daem5502_process_db_err;
			}
	...
	ZzLOG(ALWAY, "daem5502_process: 백업된 자료의 갯수는 %ld 입니다.\n",dwCount);		
	return 0;
...
int daem5502_process_init(int argc, char **argv)
{
    ...
    */
	...
	{
		ZzLOG(ERROR, "[daem5502]DB에 접속하지 못 하였습니다...\n");
		db_disconnect(con);
	   	return(-1); 
	}
