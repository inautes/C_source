...
int daem5602_process_db()
{
	...
	// lZangsiCount
	sprintf(szQuery," select count(*) from zangsi.T_INOUT_AMOUNT where inout_date = '%s' and proc_date ='1' " , gst_date );			
	...
	// lZangsiSumCount
	memset (szQuery, 0x00, sizeof(szQuery));
	sprintf(szQuery," select count(*) from zangsi_sum.T_INOUT_AMOUNT where inout_date = '%s'  " , gst_date );			
	...
	while(1) 
	{
		...
		//--------------------------------------------------------------------------
			...
			memset (szQuery, 0x00, sizeof(szQuery));
			sprintf(szQuery, "DELETE FROM zangsi.T_INOUT_AMOUNT where inout_date = '%s' and proc_date ='1' limit 100" ,gst_date);

			printf("%s\n",szQuery);

			if (mysql_query(con, szQuery))
			{
				ZzLOG(ERROR, "daem5602_delete_data: DELETE T_INOUT_AMOUNT error...\n");
