...
int  daem8001_init_process();
int  daem8001_main_process(int argc,char** argv);
int  daem8001_term_process();
void daem8001_signal(int nSignal);
int daem8001_db_select(char* pserver_id, char* pfile_path , char* pfile_name);
int daem8001_createdb_date(char *szStopDatePath);
				...
				if((strlen(szrootfullpath) >= 32) || (ZzCharCount(szrootfullpath, '/') == 7)) // 원하는 패스일경우
				{
					...
					for (nj = 0; nj < nSubCnt; nj++)
				    {
					 	...
					 	if(stat == 0 && (strcmp(".",Subdirentp[nj]->d_name) !=0 ) && (strcmp("..",Subdirentp[nj]->d_name) != 0) && (GetFistCharIndex(Subdirentp[nj]->d_name,'.') != 0))
						{
							...
							{
								int nResult = 0;
								nResult = daem8001_db_select(pServerID,szfullpath,Subdirentp[nj]->d_name);
				...
				if((strlen(szrootfullpath) >= 32) || (ZzCharCount(szrootfullpath, '/') == 7)) // 원하는 패스일경우
				{
					...
					while( (pent = readdir(pdir)) != NULL ) // /raid/fdata/2004/10/10/10 이후 부터 얻어낸다.
					{
					 	...
					 	if(stat == 0 && (strcmp(".",pent->d_name) !=0 ) && (strcmp("..",pent->d_name) != 0) && (GetFistCharIndex(pent->d_name,'.') != 0))
						{
							...
							{
								int nResult = 0;
								nResult = daem8001_db_select(pServerID,szfullpath,pent->d_name);
...
int daem8001_main_process(int argc,char** argv)
{
	...
	if (daem8001_createdb_date(szStopDatePath) != 0)
	{
		return -1;
	}
...
int daem8001_db_select(char* pserver_id, char* pfile_path , char* pfile_name)
{
	...
	sprintf(szQuery, "SELECT count(*) FROM zangsi_sum.T_CONTENTS_FILE_DEL_TEMP WHERE file_path  = '%s' AND file_name  = '%s' ; /* daem8001 */ "
	                 , pfile_path
	                 , pfile_name
	                 );
...
int daem8001_createdb_date(char *szStopDatePath)
{
	...
	memset (szQuery, 0x00, sizeof(szQuery));
	sprintf(szQuery ,"select minor_name from zangsi.T_MINOR_CODE where major_code = '100' and minor_code = '02' ; /* daem8001 */ ");
	if (mysql_query(con, szQuery)){
	    ZzLOG(ERROR, "daem8001_createdb_date: zangsi.T_MINOR_CODE error...\n");
		ZzLOG(ERROR, "[%d](%s)",mysql_errno(con), mysql_error(con));
		return -1;
	}

	if (!(res = mysql_store_result(con))) {
	    ZzLOG(ERROR, "daem8001_createdb_date: zangsi.T_MINOR_CODE mysql_store_result error...\n");
		ZzLOG(ERROR, "[%d](%s)",mysql_errno(con), mysql_error(con));
		return -1;
	}
 	...
 	{
		mysql_free_result(res);
	    ZzLOG(ALWAY, "daem8001_createdb_date: zangsi.T_MINOR_CODE 처리할 자료가 없습니다.\n");
	    return -1;
	}
	row = mysql_fetch_row(res);
	...
	memset (szQuery, 0x00, sizeof(szQuery));
	sprintf(szQuery ,"select count(*) as cnt from zangsi_sum.T_CONTENTS_FILE_DEL_TEMP ; /* daem8001 */");

	if (mysql_query(con, szQuery)){
	    ZzLOG(ERROR, "daem8001_createdb_date: zangsi_sum.T_CONTENTS_FILE_DEL_TEMP error...\n");
		ZzLOG(ERROR, "[%d](%s)",mysql_errno(con), mysql_error(con));
		return -1;
	}

	if (!(res = mysql_store_result(con))) {
	    ZzLOG(ERROR, "daem8001_createdb_date: zangsi_sum.T_CONTENTS_FILE_DEL_TEMP mysql_store_result error...\n");
		ZzLOG(ERROR, "[%d](%s)",mysql_errno(con), mysql_error(con));
		return -1;
	}
 	if (mysql_num_rows(res)==0)
 	{
		mysql_free_result(res);
	    ZzLOG(ALWAY, "daem8001_createdb_date: zangsi_sum.T_CONTENTS_FILE_DEL_TEMP 처리할 자료가 없습니다.\n");
	    return -1;
	}
	row = mysql_fetch_row(res);
...
//////////////////////////////////////////////////////////////////////////////////////
	...
	memset (szQuery, 0x00, sizeof(szQuery));
	sprintf(szQuery, "SELECT minor_name FROM zangsi.T_MINOR_CODE WHERE major_code  = '100' AND minor_code  = '01' ; /* daem8001 */ " );
...
int daem8001_init_process()
{
    ...
    ** 전역변수 초기화
    */
    ZzInitGlobalVariable2("daem8001", "/logs");

    ZzLOG(ALWAY, "[daem8001]*****************프로그램 시작*****************\n");
    ZzLOG(ALWAY, "[daem8001] 파일검색  삭제처리\n");

	printf("daem8001_init_process...\n");
	//--------------------------------------------------------------------------
	// DB 연결
	//--------------------------------------------------------------------------
...
int main(int argc, char **argv)
{
	...
	*/
	...
	signal(SIGTERM, daem8001_signal);
	signal(SIGINT,  daem8001_signal);
	signal(SIGQUIT, daem8001_signal);
	signal(SIGKILL, daem8001_signal);
	...
	if ( daem8001_init_process() > 0 )
	{
		/* 프로그램 메인루틴 */

		rc = daem8001_main_process(argc,argv);
		/* 프로그램 종료루틴 */
	}
	daem8001_term_process();
