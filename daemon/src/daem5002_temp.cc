...
int daem5002_reset_cpr_enddate();
int daem5002_reset_adult_enddate();
...
int daem5002_main_process()
{
	...
	{
		ZzLOG(ALWAY, "daem5002_cpr_update_process error. \n");
		return -1;
	}	
	ZzLOG(ALWAY, "daem5002_cpr_update_process end\n" );	
		
	daem5002_reset_cpr_enddate(); // 2014.11.27 :제휴 컨텐츠 게시기간 늘리기
	daem5002_reset_adult_enddate(); // 2016.02.11 : adult 카테고리 게시기간 늘리기
	...
	ZzLOG(ALWAY, "daem5002_set_cpr_date end\n" );	
	//daem5002_set_om_date(); //no.880
	
	//--------------------------------------------------------------------------
	// 처리일자 -7의 거래건수 만클 게시기간 연장처리
	//--------------------------------------------------------------------------
	ZzLOG(ALWAY, "daem5002_inc_end_date start\n" );	
	if (daem5002_inc_end_date() != 0)
	{
		ZzLOG(ALWAY, "daem5002_inc_end_date error. \n");
		return -1;
	}
	ZzLOG(ALWAY, "daem5002_inc_end_date end\n" );	
		
	//--------------------------------------------------------------------------
	// 처리일자 -1의 게시종료 자료를 삭제 처리한다.
	//--------------------------------------------------------------------------
	ZzLOG(ALWAY, "daem5002_del_end_date start\n" );	
	daem5002_del_end_date(); //test
	ZzLOG(ALWAY, "daem5002_del_end_date end\n" );	

	ZzLOG(ALWAY, "daem5002_flog_data_del_end_date start\n" );			
	daem5002_flog_data_del_end_date(); //test
	ZzLOG(ALWAY, "daem5002_flog_data_del_end_date end\n" );		
...
int daem5002_cpr_update_process()
{
	...
	/*	cprDB 작업 테이블 삭제	*/
	memset(szQuery, 0x00, sizeof(szQuery));
	strcpy(szQuery, "delete from zangsi_cpr.T_CPR_CONT_DEAL ; /* daem5002 */ ");
	...
	while(1)
	{
		/* 1. 게시기간 만료일자가 -3일인 영화(01), 드라마(02), 동영상(03), 애니(05),11 분류의 컨텐츠 선별하여	cprDB 작업 테이블에 저장. 	*/
		
		memset(szQuery, 0x00, sizeof(szQuery));
		sprintf(szQuery, " select a.id as id, a.sect_code as sect_code , b.vir_id" 
						 " from zangsi.T_CONTENTS_INFO a, zangsi.T_CONTENTS_VIR_ID b "
						 " where b.vir_id > %lu and a.sect_code in('01', '02', '03', '05') "
						 " and a.disp_end_date = date_format(date_add(now(), INTERVAL 3 DAY),'%%Y%%m%%d') "
						 " and a.del_yn = 'N' "
						 " and a.id = b.id and b.copyright_yn in('C','H') order by b.vir_id limit 1 ; /* daem5002 */ "
						, ulVirID);
		ZzLOG(ALWAY, "query=[%s]\n", szQuery); //test

		if(IsConnectDB(con_bck) == false)
		{
			if (!(con_bck=db_connect_backup("")))
			{
		...
		while(row = mysql_fetch_row(res))
		{
			...
			ulVirID = (unsigned long)getnum(row, 2);
			...
			*/
			...
			sprintf(szQuery, " select count(deal_no) "
							 ...
							 " deal_date >= date_format(date_add(now(), INTERVAL -45 DAY),'%%Y%%m%%d') ; /* daem5002 */ "
							 , ulID);
	...
	for(int i=0;i<4;i++)
	{
		...
		if(i == 0)	
		{
			strcpy(szQuery, " select id from zangsi_cpr.T_CPR_CONT_DEAL "
							" where sect_code = '01' "
							" group by id order by id ; /* daem5002 */");
		}
		else if(i == 1)
		{
			strcpy(szQuery, " select id from zangsi_cpr.T_CPR_CONT_DEAL "
...
int daem5002_inc_end_date()
{
	...
	//--------------------------------------------------------------------------
	// 처리일자 -7일의 판매건수를 처리할 임시테이블 zangsi_bck.T_CONTENTS_DEAL_COUNT 초기화 한다.
	//--------------------------------------------------------------------------
	memset (szQuery, 0x00, sizeof(szQuery));
	sprintf(szQuery, " delete from zangsi_bck.T_CONTENTS_DEAL_COUNT ; /* daem5002 */ ");
	...
	if (mysql_query(con_bck, szQuery)){
		ZzLOG(ERROR, "daem5002_inc_end_date con_bck : [%d](%s)\n",mysql_errno(con_bck), mysql_error(con_bck));
		tran_rollback(con_bck);
		tran_end(con_bck);
		return -1;	
    }

	//--------------------------------------------------------------------------
	// 만료일자 0일인 컨텐츠의 7일동안 판매 개수를 임시로 넣어준다.
	//--------------------------------------------------------------------------
	memset (szQuery, 0x00, sizeof(szQuery));
	sprintf(szQuery, "insert zangsi_bck.T_CONTENTS_DEAL_COUNT(id, deal_cnt, reg_date, reg_time) "
				" SELECT a.id, count(a.id) cnt , date_format(now(),'%%Y%%m%%d'), date_format(now(),'%%H%%i%%s') "
				" FROM zangsi.T_DEAL_INFO a  "
				" , zangsi.T_CONTENTS_INFO b  "
				" WHERE a.deal_date >= date_format(date_add(now(), interval -7 day),'%%Y%%m%%d') "
				" AND a.cont_gu = 'WE'  "
				" AND a.id = b.id  "
				" AND b.del_yn = 'N' " 
				" AND b.disp_end_date <= date_format(now(),'%%Y%%m%%d') "
				" GROUP BY a.id  ; /* daem5002 */ ");
	...
	while(1)
	{
		...
		//--------------------------------------------------------------------------
		// 만료일자 0일인 컨텐츠의 id, 개수를 조회
		//--------------------------------------------------------------------------


		MYSQL_RES*	bck_res = NULL;
		MYSQL_ROW	bck_row = NULL;

		memset (szQuery, 0x00, sizeof(szQuery));
		sprintf(szQuery, " select a.id, a.deal_cnt from zangsi_bck.T_CONTENTS_DEAL_COUNT a where a.id > %lu limit 1 ; /* daem5002 */ ",cont_id);
		...
		if (!(bck_res = mysql_store_result(con_bck))) 
		{
			ZzLOG(ERROR, "[%d](%s)",mysql_errno(con_bck), mysql_error(con_bck));
			return -1;
		}
		...
		while( (bck_row=mysql_fetch_row(bck_res)) != NULL )
		{
			cont_id = (unsigned long)getnum(bck_row,0);
			deal_cnt = (unsigned long)getnum(bck_row,1);
			...
			memset (szQuery, 0x00, sizeof(szQuery));
			sprintf(szQuery, "  update zangsi.T_CONTENTS_INFO "
							" set disp_end_date = date_format(date_add(now(), interval + %lu day ),'%%Y%%m%%d') "
							" where id = %lu ; /* daem5002 */ " ,deal_cnt,cont_id );
			...
			//--------------------------------------------------------------------------
			memset (szQuery, 0x00, sizeof(szQuery));
			sprintf(szQuery, "  insert into zangsi.T_CONTENTS_CREATE(id, cont_gu, udt_cd) values( %lu , '01', 'U') ; /* daem5002 */ " , cont_id );
...
daem5002_inc_end_date_err:
	ZzLOG(ERROR, "daem5002_inc_end_date con : [%d](%s)\n",mysql_errno(con), mysql_error(con));
	tran_rollback(con);
	tran_end(con);
	return -1;	
}
...
int daem5002_del_end_date()
{
	...
	strcat (szQuery, "   AND a.id      = b.id  ; /* daem5002 */ ");
	...
	strcat (szQuery, " WHERE a.reg_user='moremusic' and a.disp_end_date <= date_format(date_add(now(), INTERVAL 1 DAY),'%Y%m%d') ; /* daem5002 */ ");
...
*/
	...
	//T_CONTENTS_BEST 에 등록되어 있고 삭제된 컨텐츠에 대한 처리
	{
		...
		sprintf(szQuery,
						"SELECT a.id ,a.reg_user ,b.best_gu "
				         ...
				         "   AND a.del_yn        = 'N'          "
				         "   AND a.id = b.id and b.best_gu in('HK' ,'HS','HD') ; /* daem5002 */ "
				         ,gdis_date
				         );
		...
		while( (best_row=mysql_fetch_row(best_res)) != NULL )
		{
			...
			if( strcmp(best_gu,"HK") == 0 )
			{
				...
				sprintf(szQuery, " update zangsi.T_CONTENTS_INFO a, zangsi.T_CONTENTS_VIR_ID b "
				                 ...
				                 " , b.copyright_yn= 'T'  "
				                 " where a.id = b.id and a.disp_stat = 'Y' and a.disp_end_date <= '%s' and a.del_yn = 'N' and a.id = %lu ; /* daem5002 */ "
				                 ,gdis_date,cont_id
				                 );
			...
			sprintf(szQuery," select default_hash from zangsi.T_CONTENTS_FILELIST_SUB where id = %lu and file_size > 10485760 ; /* daem5002 */ ",cont_id);
			...
			{
				sprintf(szQuery," select id from zangsi.T_CONTENTS_FILELIST_SUB where id != %lu and default_hash = '%s' limit 1 ; /* daem5002 */ ",cont_id,getstr(hash_ori_row,0) );
    ...
    //memo가 50개 이상일 경우 처리 
    {
		...
		while( (best_row=mysql_fetch_row(best_res)) != NULL )
		{
			...
			sprintf(szQuery, "INSERT INTO zangsi.T_CONTENTS_BEST  ( best_gu, id ,reg_date,reg_time) values ( 'HK' ,%lu ,'%s','%s') ; /* daem5002 */ "
			 				 ,cont_id
			 				 ,gsys_date
			                 ,gsys_time
			                 );
    ...
    //일반 로직 처리  - 위에서 처리하지 않는 모든것들을 처리 한다.
    {
		...
		while( (best_row=mysql_fetch_row(best_res)) != NULL )
		{
			...
			sprintf(szAddQuery,  "insert into zangsi.T_IMG_DEL  "
	 		...
	 		sprintf(szQuery, "%s and b.disp_end_date <= '%s'  and b.id = %lu ; /* daem5002 */",szAddQuery,gdis_date,cont_id);
			ZzLOG(ALWAY, "zangsi.T_IMG_DEL3 Query [ %s ] \n",szQuery);
			...
			{
				sprintf(szQuery,"delete from zangsi.T_CONTENTS_IMG where cont_gu='WE' and reg_user = '%s' and id = %lu ; /* daem5002 */ ",szRegUser,cont_id );
	...
	strcpy(szQuery, " select count(a.id)  "
					...
					" where a.id = b.id ; /* daem5002 */ "); // 관리자 디비 조회
	...
	memset(szQuery, 0x00, sizeof(szQuery));
	sprintf(szQuery, " update zangsi.T_MINOR_CODE set minor_name = '%d' where major_code = '999' and minor_code = '99' ; /* daem5002 */ "
					 , nCnt);
	...
	while((row = mysql_fetch_row(res))) 
	{
		...
		strcpy( szAddQuery, "insert into zangsi.T_IMG_DEL  "
		                ...
		                "		   , file_path1, file_path2, file_name, reg_user, reg_date, reg_time  )"
		                 ...
		                 " WHERE a.cont_gu = 'FD' and a.reg_user = b.reg_user and a.id = b.id  and b.disp_stat = 'Y'  and b.del_yn = 'N' and a.img_no > 0  ; /* daem5002 */ " 
		                 );
		sprintf(szQuery, "%s and b.disp_end_date <= '%s' ",szAddQuery, gdis_date);
		ZzLOG(ALWAY, "zangsi.T_IMG_DEL2 Query [ %s ] \n",szQuery);
		...
		strcpy(szAddQuery, "select id ,reg_user from zangsi.T_CONTFLOG_INFO where disp_stat='Y' and del_yn='N' ; /* daem5002 */  ");
		...
		while( row = mysql_fetch_row(res_local))
		{
			...
			{
				memset(szQuery,0x00,sizeof(szQuery));
				sprintf(szQuery,"delete from zangsi.T_CONTFLOG_IMG where cont_gu='FD' and reg_user = '%s' and id = %lu ; /* daem5002 */ ",szRegUser,id );
...
int daem5002_reset_cpr_enddate()
{
    ZzLOG(ALWAY, "daem5002_reset_cpr_enddate start!!\n");
	...
	{
		memset (szQuery, 0x00, sizeof(szQuery));
		// 1. 제휴 컨텐츠중 게시기간 만료 날자가 오늘 이하인 목록을 뽑느다.
		sprintf(szQuery, " select a.vir_id, b.id, b.sect_code, b.sect_sub, b.adult_yn, a.copyright_yn, b.reg_user, b.title "
						 ...
						 " where a.id = b.id and a.copyright_yn in('C', 'H') and a.vir_id > %lu "
						 " and b.del_yn = 'N' "
						 " and b.disp_end_date <= date_format(now(),'%%Y%%m%%d') "
						 " order by a.vir_id "
						 " limit 1 ; /* daem5002 */ ", ulVirID);  // 관리자 디비 조회

		
		ZzLOG(ALWAY, "daem5002_reset_cpr_enddate : [ szQuery : %s ]\n",szQuery);
		
		if (mysql_query(con_bck, szQuery))
		{
			ZzLOG(ERROR, "Query [ %s ]\n",szQuery);
		    ZzLOG(ERROR, "daem5002_reset_cpr_enddate_err: UPDATE T_CONTENTS_INFO error...\n");
		    goto daem5002_reset_cpr_enddate_err;
	    }
		...
		if (!(res = mysql_store_result(con_bck))) 
		{
	 	...
	 	{
			mysql_free_result(res);
		    ZzLOG(ALWAY, "daem5002_reset_cpr_enddate: 처리할 자료가 없습니다.\n");
			return 0;
		}
		...
		while((row = mysql_fetch_row(res))) 
		{
			...
			// 유일해쉬 체크
			//--------------------------------------------------------------------------
			//유일해쉬 확인을 위한 해쉬값 가져오기
			sprintf(szQuery," select default_hash from zangsi.T_CONTENTS_FILELIST_SUB where id = %lu and file_size > 10485760 ; /* daem5002 */ ",ulID);
			...
			{
				//유일해쉬 확인
				sprintf(szQuery," select a.id from zangsi.T_CONTENTS_FILELIST_SUB a , zangsi.T_CONTENTS_VIR_ID b where a.id=b.id and b.copyright_yn in ('C','H') and a.id != %lu and a.default_hash = '%s' limit 1 ; /* daem5002 */ ",ulID,getstr(hash_ori_row,0) );
				...
				if(mysql_num_rows(hash_res) == 0)
				{
					...
					memset(szQuery, 0x00, sizeof(szQuery));
					sprintf(szQuery, " update zangsi.T_CONTENTS_INFO  "
 									 " set disp_end_date = '99901231'  " // 2015.06.01 
									 " where id = %lu and del_yn = 'N' ; /* daem5002 */ "
									 , ulID);
...
//									 " (select count(*) from zangsi.T_CONTENTS_FILELIST_SUB a, zangsi.T_CONTENTS_INFO b ,zangsi.T_CONTENTS_VIR_ID c where a.id=b.id and b.id=c.id and b.del_yn='N' and  a.default_hash = '%s' and b.disp_end_date <= date_format(now(),'%%Y%%m%%d') and c.copyright_yn in ('C','H')) A,  "
//									 " (select count(*) from zangsi.T_CONTENTS_FILELIST_SUB a, zangsi.T_CONTENTS_INFO b ,zangsi.T_CONTENTS_VIR_ID c where a.id=b.id and b.id=c.id and b.del_yn='N' and  a.default_hash = '%s' and b.disp_end_date > date_format(now(),'%%Y%%m%%d') and c.copyright_yn in ('C','H')) B ; /* daem5002 */"

					sprintf(szQuery, " SELECT SUM(IF( IFNULL(b.disp_end_date,0) <= date_format(now(), '%%Y%%m%%d'),1,0)) AS A "
									" , SUM(IF( IFNULL(b.disp_end_date,0) > date_format(now(), '%%Y%%m%%d'),1,0)) AS B "
									" FROM zangsi.T_CONTENTS_FILELIST_SUB a,  zangsi.T_CONTENTS_INFO b, zangsi.T_CONTENTS_VIR_ID c "
									" WHERE a.id = b.id AND b.id = c.id AND b.del_yn = 'N' AND a.default_hash = '%s' AND c.copyright_yn NOT IN ('C', 'H'); /* daem5002 */ "
									 , getstr(hash_ori_row,0) );
					
					MYSQL_RES* count_res = NULL;
				    MYSQL_ROW count_row = NULL;
					unsigned long uACount1=0; // 오늘 기준 만료날자 컨텐츠 카운트
					unsigned long uBCount2=0; // 오늘 이후 기준 만료날자 컨텐츠 카운트 
					if(mysql_query(con_bck, szQuery))
					{
						ZzLOG(ERROR, "Query [ %s ]\n",szQuery);		
						goto daem5002_reset_cpr_enddate_err;		
					...
					if(!(count_res = mysql_store_result(con_bck)))
					{
						goto daem5002_reset_cpr_enddate_err;
					}
					...
					else
					{
						...
						uACount1 = (unsigned long)getnum(count_row, 0);
						uBCount2 = (unsigned long)getnum(count_row, 1);
					}
					mysql_free_result(count_res);

								// 2. B 가 있다면 SKIP
					// 2. B 가 없고, A가 2보다 크다면 하나만 업데이트
					if(uBCount2 == 0 && uACount1 > 1)
					{
						memset(szQuery, 0x00, sizeof(szQuery));
						// 게시물 당 등록 파일수가 작은 컨텐츠 id를 가져온다.
						sprintf(szQuery, " select a.id "
										" from zangsi.T_CONTENTS_FILELIST_SUB a , (select id from zangsi.T_CONTENTS_FILELIST_SUB where default_hash ='%s') b "
										" where a.id=b.id  group by a.id order by count(*) asc limit 1 ; /* daem5002 */"
										, getstr(hash_ori_row,0) );
						
						MYSQL_RES* uid_res = NULL;
						MYSQL_ROW uid_row = NULL;
						unsigned long uId_Contents=0; 
						...
						if(!(uid_res = mysql_store_result(con)))
						{
							goto daem5002_reset_cpr_enddate_err;
						}

						if(mysql_num_rows(uid_res) == 0)
						{
						}
						else
						{
							uid_row = mysql_fetch_row(uid_res);
							uId_Contents = (unsigned long)getnum(uid_row, 0);
...
daem5002_reset_cpr_enddate_err:
	ZzLOG(ERROR, "daem5002_reset_cpr_enddate_err: [%d](%s)\n",mysql_errno(con), mysql_error(con));
	mysql_free_result(res);
	return -1;	
}
...
int daem5002_reset_adult_enddate()
{
    ZzLOG(ALWAY, "daem5002_reset_adult_enddate start!!\n");
	...
	{
		memset (szQuery, 0x00, sizeof(szQuery));
		// 1. sect_code = 11 비제휴, 게시기간 만료 날자가 오늘 이하인 목록을 뽑는다.
		sprintf(szQuery, " select a.vir_id, b.id, b.sect_code, b.sect_sub, b.adult_yn, a.copyright_yn, b.reg_user, b.title "
						 " from zangsi.T_CONTENTS_VIR_ID a, zangsi.T_CONTENTS_INFO b "
						 " where a.id = b.id and a.sect_code = 11 and a.copyright_yn not in ('C', 'H') and a.vir_id > %lu "
						 " and b.del_yn = 'N' "
						 " and b.disp_end_date <= date_format(now(),'%%Y%%m%%d') "
						 " order by a.vir_id "
						 " limit 1 ; /* daem5002 */ ", ulVirID); // 관리자 db 조회

		
		ZzLOG(ALWAY, "daem5002_reset_adult_enddate : [ szQuery : %s ]\n",szQuery);
		
		if (mysql_query(con_bck, szQuery))
		{
			ZzLOG(ERROR, "Query [ %s ]\n",szQuery);
		    ZzLOG(ERROR, "daem5002_reset_adult_enddate_err: UPDATE T_CONTENTS_INFO error...\n");
		    goto daem5002_reset_adult_enddate_err;
	    }
		...
		if (!(res = mysql_store_result(con_bck))) 
		{
	 	...
	 	{
			mysql_free_result(res);
		    ZzLOG(ALWAY, "daem5002_reset_adult_enddate: 처리할 자료가 없습니다.\n");
			return 0;
		}
		...
		while((row = mysql_fetch_row(res))) 
		{
			...
			{
				ZzLOG(ERROR, "Query [ %s ]\n",szQuery);
			    ZzLOG(ERROR, "DeleteContents2: INSERT T_CONTENTS_DEL error...\n");
			    ZzLOG(ERROR, "DeleteContents2: [%d](%s)\n", mysql_errno(con), mysql_error(con));
				goto daem5002_reset_adult_enddate_err;
			...
			if(!(hash_ori_res = mysql_store_result(con)))
			{
				ZzLOG(ERROR, "DeleteContents2: mysql_store_result error [%s]\n", szQuery);
				ZzLOG(ERROR, "DeleteContents2: [%d](%s)\n", mysql_errno(con), mysql_error(con));
				goto daem5002_reset_adult_enddate_err;
			}
			...
			{
				//유일해쉬 확인
				sprintf(szQuery," select a.id from zangsi.T_CONTENTS_FILELIST_SUB a , zangsi.T_CONTENTS_VIR_ID b where a.id=b.id and b.copyright_yn not in ('C','H') and a.id != %lu and a.default_hash = '%s' limit 1 ; /* daem5002 */ ",ulID,getstr(hash_ori_row,0) );
				...
				else
				{
					...
					memset(szQuery, 0x00, sizeof(szQuery));
					sprintf(szQuery, " select "
									 " (select count(*) from zangsi.T_CONTENTS_FILELIST_SUB a, zangsi.T_CONTENTS_INFO b ,zangsi.T_CONTENTS_VIR_ID c where a.id=b.id and b.id=c.id and b.del_yn='N' and  a.default_hash = '%s' and b.disp_end_date <= date_format(now(),'%%Y%%m%%d') and c.copyright_yn not in ('C','H')) A,  " // 1
									 " (select count(*) from zangsi.T_CONTENTS_FILELIST_SUB a, zangsi.T_CONTENTS_INFO b ,zangsi.T_CONTENTS_VIR_ID c where a.id=b.id and b.id=c.id and b.del_yn='N' and  a.default_hash = '%s' and b.disp_end_date > date_format(now(),'%%Y%%m%%d') and c.copyright_yn not in ('C','H')) B ; /* daem5002 */ " // 0 
									 , getstr(hash_ori_row,0),getstr(hash_ori_row,0));
					...
					if(!(count_res = mysql_store_result(con)))
					{
						goto daem5002_reset_adult_enddate_err;
					}
...
daem5002_reset_adult_enddate_err:
	ZzLOG(ERROR, "daem5002_reset_adult_enddate_err: [%d](%s)\n",mysql_errno(con), mysql_error(con));
	mysql_free_result(res);
	return -1;	
}
...
int daem5002_set_cpr_date()
{
	...
	while(1)
	{
		...
		sprintf(szQuery, " select a.vir_id, b.id, b.sect_code, b.sect_sub, b.adult_yn, a.copyright_yn, b.reg_user, b.title "
						 ...
						 " and b.disp_end_date <= date_format(now(),'%%Y%%m%%d') "
						 " order by a.vir_id "
						 " limit 1 ; /* daem5002 */ ", ulVirID); // 관리자 db.
...
int daem5002_get_sysdate()
{
	...
	if (strcmp(gproc_date, "00000000")==0) {
		strcpy(szQuery, "SELECT date_format(date_add(now(), INTERVAL -7 DAY),'%Y%m%d')"); // 2015.06.01 
		strcat(szQuery, "     , date_format(date_add(now(), INTERVAL  4 DAY),'%Y%m%d')");
		strcat(szQuery, "     , date_format(now(),'%Y%m%d')");
		strcat(szQuery, "     , date_format(now(),'%Y%m%d')");
		strcat(szQuery, "     , date_format(now(),'%H%i%s') ; /* daem5002 */ ");
	}
	else
	{
		sprintf(szQuery, "SELECT date_format(date_add('");
		strcat (szQuery, gproc_date);
		strcat (szQuery, "', INTERVAL -7 DAY),'%Y%m%d')"); // 2015.06.01 
