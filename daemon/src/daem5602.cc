...
int daem5602_process()
{
	...
	{
		gbIsUserDate = false;
		strcpy(szQuery, "SELECT date_format(date_add(now(), INTERVAL - 14 DAY),'%Y%m%d') ; /*daem5602*/ ");
	...
	while(1) 
	{
		...
		while((row = mysql_fetch_row(res))) 
		{
		    ...
		    if(bIsSuceed == true || (bIsSuceed == false && nMysqlErrno == 1062))
		    {
				...
				sprintf(szQuery, "DELETE FROM zangsi.T_INOUT_AMOUNT "
								 "where user_id = '%s' and inout_date = '%s' and seq_no = %ld ; /*daem5602*/  "
