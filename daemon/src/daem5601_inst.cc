...
int daem5601_process()
{
	...
	{
		gbIsUserDate = false;
		strcpy(szQuery, "SELECT date_format(date_add(now(), INTERVAL - 8 DAY),'%Y%m%d')");
		// 현재로부터 4일 이전 날짜를 얻는다.
...
//while
		while((row = mysql_fetch_row(res))) 
		{
			...
			lFirstNo = (unsigned long)getnum(row, 0);
...
int daem5601_process_db()
{
	...
	sprintf(szQuery," select max(deal_no) "
				" from zangsi_sum.T_DEAL_INFO "
				" where deal_date = '%s' "
				, gst_date ,lFirstNo
				);		
	...
	printf(" Date = %s ,lFirstNo = %ld \n",gst_date,lFirstNo);
	ZzLOG(ALWAY, " Date = %s ,lFirstNo = %ld \n",gst_date,lFirstNo);
