...
int cmds1001(char *pRecvHead, char *pRecvData, char* &pSendData)
{
	...
	if(find_flag == 2) // 20100209 -- HCS : 내디스크도 일반 컨텐츠서버로...
		...
		sprintf(szServerGu , "0%d" ,find_flag);
	...
	{
		//버전검사
		//infLOG(ALWAY,"버전검사 [ %d ] == [ %d ] \n",nUpVersion,(12305+12306));//12426
		infLOG(ALWAY,"버전검사 [ %d ] == [ %d ] \n",nUpVersion,(12605+12603));//12426
		//if( nUpVersion < (12305+12306) )
		if( nUpVersion < (12605+12603) )//24852
		{
			ErrNum = -10000;
			//sprintf(ErrMsg, "최신 모듈이 아닙니다.모듈을 업데이트 해주세요.\n");
			//sprintf(ErrMsg, "업로드 모듈이 손상되었습니다.삭제 후 다시 설치해 주십시오\n");
			//sprintf(ErrMsg, "");
			sprintf(ErrMsg, "최신 모듈이 아닙니다.삭제 후 다시 설치해 주십시오.\n");
			infLOG(ERROR, "cmds1001->ERRMSG Ver : [%d] [%s]\n",ErrNum,ErrMsg);
			E_dump(ErrNum, ErrMsg, pSendData);
			db_disconnect(con);
		   	return(ErrNum);
		}
	...
	if (find_flag == 1 || find_flag == 3 || find_flag == 2 )
	{
		...
		//
		//우선순위 20인 서버 질의
		nRowcnt = GetPriServerInfo(con, 20, szServerGu, &t_cmds1001s);
		if(nRowcnt < 0)
		{
			ErrNum = -10012;
			sprintf(ErrMsg, "검색시 오류가 발생하였습니다.\n");
			infLOG(ERROR, "cmds1001->ERRMSG: [%d]\n",ErrNum);
		...
		//운선순위 50인 서버 질의
		nRowcnt = 0;
		nRowcnt = GetPriServerInfo(con, 50, szServerGu, &t_cmds1001s);
		if(nRowcnt < 0)
		{
			ErrNum = -10013;
			sprintf(ErrMsg, "검색시 오류가 발생하였습니다.\n");
...
int GetPriServerInfo(MYSQL *con, int nLimit, char* pServerGu, LPCCMDS1001_S pCmd1001_S)
{
	...
	if(nLimit == 20)
	{
		sprintf(szQuery, " select a.server_id, a.server_ip, "
		                 ...
		                 " a.root_path "
		                 " from zangsi.T_SERVER_INFO a, zangsi.T_MINOR_CODE b, zangsi.T_MINOR_CODE c "
		                 " where a.server_gu = '%s' and user_cnt=20 and admin_open_yn = 'Y' and upload_yn = 'Y'  "
		                 " and b.major_code = '720' and b.minor_code = 'DN' and c.major_code = '720' and c.minor_code = 'UP' "
		                 " and dn_user < b.minor_name and up_user < c.minor_name "
		                 " order by  up_user , dn_user  limit 1 "
		        ,pServerGu);
	}
	else if(nLimit == 50)
	{
		sprintf(szQuery, "select a.server_id,   a.server_ip,  "
		                 ...
		                 " where a.server_gu = '%s' and user_cnt = 50 and admin_open_yn = 'Y' and upload_yn = 'Y' "
		                 " and dn_user < 25 and up_user < 3 "
		                 " order by  up_user , dn_user   limit 1 "
		        ,pServerGu);
	}
