...
//  com9010 main
//
//  input : pcom9010_r.proc_flag => 1=wedisk, 2=mydata
//
//  return:  1(정상)
//          -1(DB오류)
//******************************************************************************
long com9010(CCOM9010_R pcom9010_r,char* szIP, int nPort)
{
	...
	{
		infLOG(ERROR, "com9010[ERR]: 소켓 생성 오류 입니다.\n");
	...
	{
		 infLOG(ERROR, " ]com9010[ERR]중개서버 접속 오류 : [ %s ] [ %d ]  \n " ,szIP,nPort);
	...
	dcmdHeader.nDataSize = sizeof(CCOM9010_R); //unsigend long
	dcmdHeader.nCmd = 9010;
	...
	if( dcmdHeader.nDataCnt * dcmdHeader.nDataSize > 0 )
		memcpy( pSendData + HEADER_SIZE, &pcom9010_r , dcmdHeader.nDataCnt * dcmdHeader.nDataSize);

	if(ComSendData(dcmd_socket,pSendData, dwSendDataLen ) <= 0)
	{
		#ifdef __DEBUG
		printf(" ] com9010 요청 패킷 전송 \n");
		#endif
		infLOG(ERROR, "com9010 요청 패킷 전송 \n");
		...
		infLOG(ERROR, "com9010[ERR]: 9010 통신 오류(S) : 서버와의 통신 오류 입니다. 1분후 재시도 해주십시오. \n");
	...
	if(ComRecvData(dcmd_socket,(char*)&dcmdHeader,HEADER_SIZE ) <= 0)
	{
		...
		infLOG(ERROR, "com9010[ERR]: 9010 통신 오류(R) : 서버와의 통신 오류 입니다. 1분후 재시도 해주십시오. \n");
