 ...
 *   프로그램명 : com9104.cc
 *         기능 : upload 오류처리
 *         설명 : wedisk, mydata에 upload시 오류발생하면 호출된다.
 *       작성자 : JDP
...
//          -1(DB오류)
//******************************************************************************
long com9104(CCOM9104_R pcom9104_r,char* szIP, int nPort)
{
		...
		{
			infLOG(ERROR, "com9104[ERR]: 소켓 생성 오류 입니다.\n");
		...
		{
			 infLOG(ERROR, " ]com9104[ERR]중개서버 접속 오류 : [ %s ] [ %d ]  \n " ,szIP,nPort);
		...
		dcmdHeader.nDataSize = sizeof(CCOM9104_R); //unsigend long
		dcmdHeader.nCmd = 9104;
		...
		if( dcmdHeader.nDataCnt * dcmdHeader.nDataSize > 0 )
			memcpy( pSendData + HEADER_SIZE, &pcom9104_r , dcmdHeader.nDataCnt * dcmdHeader.nDataSize);

		if(ComSendData(dcmd_socket,pSendData, dwSendDataLen ) <= 0)
		{
			#ifdef __DEBUG
			printf(" ] com9104 요청 패킷 전송\n");
			#endif
			...
			infLOG(ERROR, "com9104[ERR]: 9104 통신 오류(S) : 서버와의 통신 오류 입니다. 1분후 재시도 해주십시오. \n");
		...
		if(ComRecvData(dcmd_socket,(char*)&dcmdHeader,HEADER_SIZE ) <= 0)
		{
			...
			infLOG(ERROR, "com9104[ERR]: ComRecvData 9104 통신 오류(R) : 서버와의 통신 오류 입니다. 1분후 재시도 해주십시오. \n");
		...
		if( dcmdHeader.nCmd == 9104 )
		{

		}
		else
		{
			infLOG(ERROR, "com9104[ERR]: 9104 통신 오류(R) : 서버와의 통신 오류 입니다. 1분후 재시도 해주십시오. \n");
