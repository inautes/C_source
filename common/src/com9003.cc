...
//          -3(남아있는 용량보다 사이즈가 큼)
//******************************************************************************
long com9003(CCOM9003_R pcom9003_r,char* szIP, int nPort)
{
	char szQuery[10000];
	MYSQL       *con;
	MYSQL_RES *res;
	...
	sprintf(szQuery, "UPDATE T_MYDISK_INFO        "
	                 "   SET disk_use = disk_use + %13.0f"
	                 " WHERE user_id  = '%s'            "
	                 ,pcom9003_r.file_size
	                 ,pcom9003_r.user_id
	...
	if (mysql_query(con, szQuery)){
		infLOG(ERROR, "com9003[ERR]: UPDATE T_MYDISK_INFO (%s)\n", mysql_error(con));
		infLOG(ERROR, "com9003[SQL]: %s\n", szQuery);
		db_disconnect(con);
