...
long com9202(CCOM9202_R pcom9202_r,char* szIP, int nPort)
{
	...
	if ( ( dcmd_socket = socket(PF_INET,SOCK_STREAM,IPPROTO_TCP)) < 0 )
	{
		...
		{
			 infLOG(ERROR, " ]com9202[ERR]중개서버 접속 오류 : [ %s ] [ %d ]  \n " ,szIP,nPort);
	...
	dcmdHeader.nDataSize = sizeof(CCOM9202_R); //unsigend long
	dcmdHeader.nCmd = 9202;
	...
	if( dcmdHeader.nDataCnt * dcmdHeader.nDataSize > 0 )
		memcpy( pSendData + HEADER_SIZE, &pcom9202_r , dcmdHeader.nDataCnt * dcmdHeader.nDataSize);

	if(ComSendData(dcmd_socket,pSendData, dwSendDataLen ) <= 0)
	{
		#ifdef __DEBUG
		printf(" ] com9202 요청 패킷 전송 \n");
		#endif
		infLOG(ERROR, "com9202 요청 패킷 전송 실패 \n");
		...
		infLOG(ERROR, "com9202[ERR]: 9202 통신 오류(S) : 서버와의 통신 오류 입니다. 1분후 재시도 해주십시오. \n");
	...
	if(ComRecvData(dcmd_socket,(char*)&dcmdHeader,HEADER_SIZE ) <= 0)
	{
		...
		infLOG(ERROR, "com9202[ERR]: 9202 통신 오류(R) : 서버와의 통신 오류 입니다. 1분후 재시도 해주십시오.1 \n");
	...
	if( dcmdHeader.nCmd == 9202 )
	{

	}
	else
	{
		infLOG(ERROR, "com9202[ERR]: 9202 통신 오류(R) : 서버와의 통신 오류 입니다. 1분후 재시도 해주십시오.2 \n");
