...
//          -1(DB오류)
//******************************************************************************
long com9001(CCOM9001_R pcom9001_r,char* szIP, int nPort)
{
	...
	{
		infLOG(ERROR, "com9001[ERR]: 소켓 생성 오류 입니다.\n");
	...
	if( connect(dcmd_socket,(struct sockaddr * )&dcmdSerAddr,sizeof(dcmdSerAddr)) < 0 )
	{
		...
		dcmdHeader.nDataSize = sizeof(CCOM9001_R); //unsigend long
		dcmdHeader.nCmd = 9001;
		...
		if( dcmdHeader.nDataCnt * dcmdHeader.nDataSize > 0 )
			memcpy( pSendData + HEADER_SIZE, &pcom9001_r , dcmdHeader.nDataCnt * dcmdHeader.nDataSize);

		if(ComSendData(dcmd_socket,pSendData, dwSendDataLen ) <= 0)
		{
			#ifdef __DEBUG
			printf(" ] com9001 요청 패킷 전송 \n");
			#endif
			infLOG(ERROR, "com9001 요청 패킷 전송 \n");
			...
			infLOG(ERROR, "com9001[ERR]: 9001 통신 오류(S) : 서버와의 통신 오류 입니다. 1분후 재시도 해주십시오. \n");
		...
		if(ComRecvData(dcmd_socket,(char*)&dcmdHeader,HEADER_SIZE ) <= 0)
		{
			...
			infLOG(ERROR, "com9001[ERR]: 9001 통신 오류(R) : 서버와의 통신 오류 입니다. 1분후 재시도 해주십시오. \n");
		...
		if( dcmdHeader.nCmd == 9001 )
		{

		}
		else
		{
			infLOG(ERROR, "com9001[ERR]: 9001 통신 오류(R) Cmd ( %d ) : 서버와의 통신 오류 입니다. 1분후 재시도 해주십시오. \n",dcmdHeader.nCmd);
		...
		//infLOG(ALWAY, "com9001 : 9001 통신 완료(R)\n");
