...
*/
	...
	#ifdef __DEBUG
	infLOG(ALWAY," ] UserID Check ( %s ) == ( %s )\n",dcmdHeader.szUserID,pUserID);
	printf(" ] UserID Check ( %s ) == ( %s )\n",dcmdHeader.szUserID,pUserID);
	#endif
	...
	{
		#ifdef __DEBUG
		printf("com9005 요청 패킷 전송 ID ( %s ) \n",dcmdHeader.szUserID);
		#endif
		infLOG(ERROR, "com9005 요청 패킷 전송 ID ( %s ) \n",dcmdHeader.szUserID);
		...
		infLOG(ERROR, "com9005[ERR]: 9005 통신 오류(S) : 서버와의 통신 오류 입니다. 1분후 재시도 해주십시오. \n");
	...
	printf(" ] dcmdHeader Check \n"
		   ...
		   ,dcmdHeader.nCmd,dcmdHeader.nDataCnt,dcmdHeader.nDataSize);
	#endif
