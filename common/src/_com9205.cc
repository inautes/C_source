...
//          -1(DB오류)
//******************************************************************************
long com9205(CCOM9205_R pcom9205_r,char* szIP, int nPort)
{
	infLOG(ALWAY, "com9205\n");
	/*
	//--------------------------------------------------------------------------
	...
	{
		 infLOG(ERROR, " ]com9205[ERR]중개서버 접속 오류 : [ %s ] [ %d ]  \n " ,szIP,nPort);
	...
	dcmdHeader.nDataSize = sizeof(CCOM9205_R); //unsigend long
	dcmdHeader.nCmd = 9205;
	...
	if( dcmdHeader.nDataCnt * dcmdHeader.nDataSize > 0 )
		memcpy( pSendData + HEADER_SIZE, &pcom9205_r , dcmdHeader.nDataCnt * dcmdHeader.nDataSize);

	if(ComSendData(dcmd_socket,pSendData, dwSendDataLen ) <= 0)
	{
		#ifdef __DEBUG
		printf(" ] com9205 요청 패킷 전송 \n");
		#endif
		infLOG(ERROR, "com9205 요청 패킷 전송 실패 \n");
		...
		infLOG(ERROR, "com9205[ERR]: 9205 통신 오류(S) : 서버와의 통신 오류 입니다. 1분후 재시도 해주십시오. \n");
	...
	if(ComRecvData(dcmd_socket,(char*)&dcmdHeader,HEADER_SIZE ) <= 0)
	{
		...
		infLOG(ERROR, "com9205[ERR]: 9205 통신 오류(R) : 서버와의 통신 오류 입니다. 1분후 재시도 해주십시오.1 \n");
	...
	if( dcmdHeader.nCmd == 9205 )
	{

	}
	else
	{
		infLOG(ERROR, "com9205[ERR]: 9205 통신 오류(R) : 서버와의 통신 오류 입니다. 1분후 재시도 해주십시오.2 \n");
