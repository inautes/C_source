...
{
    /*
    ** 초기화작업 서버명.cfg파일을 읽어 환경 Setting을 한다.
    ** lib_src/apconfig.c
    */
...
void infSigHandler(int nSigNo, siginfo_t *siginfo,void* p) 
{
	...
	#ifdef __DEBUG
	printf("infMainRoutine       ] infSigHandler`SigNo[%d]\n", nSigNo);
	#endif
...
int CreateTCPServerSocket(unsigned short port)
{
	...
	//			01234567890123456789]	
		printf("CreateServerSocket	]  setsockopt error SOL_SOCKET errornum ( %d )\n",errno);
		#endif					
...
int infMainRoutine(int argc,char** argv)
{
    ...
    {
        printf("Usage:  %s <SERVER PORT - 5003 >\n", argv[0]);
        exit(1);
    }
