...
long fups4004(CFUPS4004 pfups4004)
{
	infLOG(ALWAY,"------------fups4004 start-------------\n");
	...
	{
		infLOG(ERROR, "fups4004  : socket() errno [ %d ] error msg [ %s ]\n",errno,strerror(errno));
		return(-1); 
	...
	if(st != 0)
		infLOG(ALWAY, " fups4004 : 소켓 recv time out 옵션 설정 실패 errno = ( %d )\n",errno); 
	...
	if(st != 0)
		infLOG(ALWAY, "fups4004 : 소켓 send time out 옵션 설정 실패 errno = ( %d )\n",errno);

		
	if( connect(dcmd_socket,(struct sockaddr * )&dcmdSerAddr,sizeof(dcmdSerAddr)) < 0 )
	{
		infLOG(ERROR, "fups4004 : 중개서버 접속 오류 : [ %s ] [ %d ]  \n 다음 서버(%s)로 접속합니다. \n"
	...
	dcmdHeader.nDataSize = sizeof(CFUPS4004 ); //unsigend long
	dcmdHeader.nCmd = 4004;
	...
	if( dcmdHeader.nDataCnt * dcmdHeader.nDataSize > 0 )
		memcpy( pSendData + HEADER_SIZE, &pfups4004 , dcmdHeader.nDataCnt * dcmdHeader.nDataSize);	
	infLOG(ALWAY, "fups4004  : 4004 전송 sizeof(CFUPS4004) = %ld  \n",sizeof(CFUPS4004));
	
	if(SendData(dcmd_socket,pSendData, dwSendDataLen ) <= 0)
	{
		infLOG(ERROR, "fups4004  : 4004 전송 SendData() errno [ %d ] error msg [ %s ]\n",errno,strerror(errno));
		...
		infLOG(ERROR, "fups4004[ERR]: 4004 통신 오류(S) : 서버와의 통신 오류 입니다. 1분후 재시도 해주십시오. \n");
	...
	if(RecvData(dcmd_socket,(char*)&dcmdHeader,HEADER_SIZE ) <= 0)
	{
		...
		infLOG(ERROR, "fups4004  : 4004 받기 RecvData() errno [ %d ] error msg [ %s ]\n",errno,strerror(errno));
	...
	infLOG(ALWAY,"fups4004  : result - dcmdHeader.nCmd [ %d ] \n",dcmdHeader.nCmd);
	...
	{
		infLOG(ERROR, "fups4004  : 4004 결과 오류 [ %d ] \n",dcmdHeader.nCmd );
