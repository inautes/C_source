...
//static char gszKeyMap[10000];
//static unsigned long gulAutoIncNum;
...
class CMysqlPool
{
	...
	protected:
		...
		//thread_t m_check_thread_id[[100];
		
		//queue<DB_SCHEDULE> m_Schedule[100];
		//queue<DB_SCHEDULE> m_Schedule;
		CSchQueue m_Schedule;
		queue<long> m_DBPoolSchedule;
		//multimap< long, DB_INFO > m_DB_Pool;
		//queue<DB_INFO> m_DB_Pool;
		//multimap< long, DB_INFO > m_DB_Pool;
		CDBQueue m_DB_Pool;
	...
	public:
		...
		//void CreateSchedule(long thread_id); // queue 에 해당 스레드 넣기
